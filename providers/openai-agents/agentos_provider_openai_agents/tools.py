"""The tool registry: what an SDK agent may call, and what each call MEANS to the governor.
...
Every registered tool carries an `EffectClass`, and at dispatch the executor offers the
model ONLY the tools whose class the AgentOS agent declared — an undeclared tool is not
offered-and-refused, it is simply not there (the governor's tier-2/tier-3 gate has
already ruled on the declared set before the step ran). Each tool the model actually
calls is reported back as an `Effect`, so `_settle`'s undeclared-effect check remains
the backstop.

Operators register tools through the `agentos.openai_agents_tools` entry-point group:
each entry resolves to a callable returning `Iterable[ToolSpec]`. Two harmless built-ins
(`compute` class) exist so the quickstart and tests need no plugin.
"""
...
_log = logging.getLogger("agentos.provider.openai_agents")

ENTRY_POINT_GROUP = "agentos.openai_agents_tools"


@dataclass(frozen=True)
class ToolSpec:
    """A Python callable the SDK may expose to the model, with the effect class the
    governor should charge for calling it. `fn` must have type-annotated parameters and
    a docstring: the SDK derives the JSON schema the model sees from them."""
...
def load_registry(*, builtins: bool = True,
                  ...
                  entry_point_group: str | None = ENTRY_POINT_GROUP) -> ToolRegistry:
    """Built-ins + every `agentos.openai_agents_tools` entry point + `extra`. A plugin
    that fails to load is skipped with a warning naming it (same rule as executors)."""
    ...
    if entry_point_group:
        for ep in entry_points(group=entry_point_group):
            try:
                ...
                _log.warning("tool plugin %r skipped: %s", ep.name, exc)
    for spec in extra:
        registry.add(spec)
    return registry
