...
- serialization is plain JSON with explicit field names; reading an older
  `schema_version` goes through `dagentos.core.upcast`.
...
class Event(BaseModel):
    ...
    # v0.6.0, additive: tamper-evident chain (dagentos.core.integrity, C12). None on logs
...
class ChainSealed(Event):
    ...
    hold (dagentos.core.seal). Replay ignores it; `agentos verify` and the integrity endpoint
...
def from_record(record: dict[str, Any]) -> Event:
    ...
    from dagentos.core.upcast import upcast  # local import keeps module graph acyclic
