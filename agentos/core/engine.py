"""Workflow engine. Phase 0 executes synchronously in topological order with the
echo agent. Phase 1 makes execution durable (event-sourced) and asynchronous; the
public run() shape stays the same."""
...
def _topo_order(wf) -> list[str]:
    """Kahn's algorithm — also surfaces cycles defensively (definition is pre-validated)."""
...
def run_workflow(wf_name: str) -> WorkflowRun:
    ...
    try:
        for node_id in _topo_order(wf):
            ...
            if agent is None:
                raise KeyError(f"node {node_id!r} references unknown agent {node.agent!r}")
            upstream = {dep: outputs[dep] for dep in node.depends_on}
            output = run_echo(agent, upstream)   # Phase 1: AgentRouter.dispatch(...)
